...
3. Also create `composer.json` and `index.php` files, if you are deploying to Heroku.

4. Fill in the appropriate content in the composer.json and index.php files to be able to deploy your game to Heroku when it is finished.

5. While still in your local git repo, create a directory called `assets`.
6. `cd` your way into the `assets` folder, then make three additional folders: `javascript`, `css` and `images`.
...
7. Push the above changes to GitHub.

8. Choose whichever game you'd like to build. Making the Psychic game will prove less challenging than coding Hangman. However, as the challenge of the Hangman exercise provides  a more comprehensive review of this week's material, we suggest attempting that assignment first.

9. Note: There's no shame if you'd prefer submitting Psychic—it's still a proper challenge.

10. Push your selected game to `gh-pages`. Alternatively, if using Heroku:
